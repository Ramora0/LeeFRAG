...
from leefrag.model.block_attention import build_block_causal_mask_with_qa, build_prefix_causal_mask
from leefrag.utils.kv_cache_utils import apply_rope_to_cache, concat_compressed_caches, extract_doc_hidden_states
...
class NPTTrainer:
    """Next-token prediction pretraining for KV cache compression.

    Trains the Q-Former to compress document hidden states into KV caches
    that preserve enough information for general language modeling. All
    continuation tokens are supervised (unlike TwoStageTrainer which only
    supervises answer tokens).

    Stage A (no grad): Run frozen LLM on [preamble+docs | continuation] with
        block-diagonal causal mask for docs + full attention from continuation.
        Extract per-document hidden states + teacher logits over continuation.
    Stage B (grad): Q-Former compresses hidden states → KV prefix → LLM forward
        → CE loss on all continuation tokens + KL divergence against teacher.

    This is the most downstream-aligned pretraining objective for RAG: the
    compressor learns to retain information needed for generating text that
    follows the compressed context.
    """
    ...
    def __init__(
        ...
        # Optimizer (Q-Former parameters only — no LoRA for pretraining)
        ...
        if self.use_wandb:
            import wandb
            wandb.init(project="leefrag-npt", config={
    ...
    def _training_step(
        ...
        # === Stage A: Extract hidden states + teacher logits ===
        with torch.no_grad():
        ...
        # === Stage B: Compress and compute NTP loss ===
    ...
    def _stage_a(
    ...
    ) -> tuple[list[list[torch.Tensor]], torch.Tensor | None]:
        """Stage A: Run frozen LLM on [preamble+docs | continuation].

        Uses block-diagonal causal mask for doc blocks, with continuation
        tokens attending to all blocks.

        Returns:
            per_doc_hidden: Per-block hidden states for Q-Former.
            teacher_logits: Logits over continuation tokens, or None if ce_only.
        """
        ...
        attn_mask = build_block_causal_mask_with_qa(
            block_lengths, continuation_len, dtype=dtype, device=self.device
        )
    ...
    def _stage_b(
        ...
        """Stage B: Compress hidden states, run LLM, compute NTP loss.

        Loss = CE on all continuation tokens + KL divergence against teacher.
        """
        per_doc_compressed = []
        empirical_ratios = []
        for doc_hs in doc_hidden_states:
        ...
        # CE loss on all continuation tokens
    ...
    def _compute_kl_loss(
        ...
        """Compute KL divergence between teacher and student over continuation tokens.

        Unlike TwoStageTrainer which filters to answer-only tokens, this
        computes KL over ALL continuation positions (labels != -100).
        """
    ...
    def _update_lr(self, step: int):
        """Update learning rate with per-phase warm restart."""
    ...
    def verify_gradient_flow(self):
        """Verify Q-Former parameters receive gradients."""
        has_grad = False
        zero_grad_params = []
