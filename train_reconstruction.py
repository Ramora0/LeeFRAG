"""Reconstruction pretraining entry point for KV cache compression with Q-Former.

Phase 1: Train Q-Former to reconstruct documents from compressed KV caches.
Phase 2: Load pretrained Q-Former into train.py for RAG fine-tuning.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Reconstruction pretraining for Q-Former KV compressor")
    parser.add_argument("--dataset", type=str, default="hotpotqa", choices=["rag_v1", "hotpotqa"],
                        help="Dataset to train on (default: hotpotqa)")
    parser.add_argument("--output_dir", type=str, default="outputs_reconstruction")
...
def main():
    ...
    collator = ReconstructionCollator(tokenizer)
    ...
    # Train
    logger.info("Starting reconstruction pretraining...")
    trainer.train()
